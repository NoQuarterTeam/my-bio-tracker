...
export default async function Page() {
  ...
  const markerTimelines = documentMarkers.reduce(
    (acc, marker) => {
      ...
      acc[marker.name].markers.push({ ...marker, date: marker.document.date })
  ...
  const formatChartData = (markerName: string, timeline: (typeof markerTimelines)[string]) => {
    ...
    return [...timeline.markers]
      ...
      .map((marker) => ({
        ...
        fullDate: new Date(marker.date).toISOString().split("T")[0], // YYYY-MM-DD format
